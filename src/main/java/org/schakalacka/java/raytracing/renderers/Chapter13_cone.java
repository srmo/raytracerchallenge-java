...
import org.schakalacka.java.raytracing.geometry.objects.Cone;
...
public class Chapter13_cone {


    public static void main(String[] args) {

        Material coneMaterial = Material.newMaterial().color(new Color(0.2, 0.01, 0.2)).ambient(1).diffuse(1).specular(1).shininess(300).reflectivity(0).create();
        var cone = new Cone(-1,0);
        cone.setClosed(true);
        cone.setMaterial(coneMaterial);
        cone.setTransformationMatrix(MatrixProvider.translation(0,2,0));

        var world = new World(true);
        world.setLightSource(new PointLight(Tuple.point(-10, 20, -10), new Color(0.8, 0.8, 0.8)));
        world.addObjects(cone);
...
//        var height = 2160;
        ...
        PPMExporter.export(canvas, "chapter13_cone_%dx%d_chunks_%d_Matrix_%s.ppm".formatted(width, height, parallelChunks, MatrixProvider.MT), 255);
